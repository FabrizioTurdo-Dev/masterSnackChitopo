...
export default function PageHeader({ eyebrow, title, subtitle, children }) {
  return (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-7">
      <div>
        {eyebrow && (
          <p className="font-condensed uppercase tracking-[0.22em] text-ink text-xs m-0 mb-2 before:content-['★'] before:text-fire before:mr-2">
            {eyebrow}
          </p>
        )}
        <h2 className="font-title uppercase text-4xl sm:text-5xl leading-[0.92] text-ink m-0">{title}</h2>
        {subtitle && <p className="text-sm text-ink-soft mt-2 mb-0">{subtitle}</p>}
      </div>
