...
export function QtyStepper({ qty, max, onChange, size = "md" }) {
  ...
  const btn = `${box} grid place-items-center text-ink hover:bg-cream-2 transition-colors cursor-pointer disabled:opacity-35 disabled:cursor-not-allowed disabled:hover:bg-transparent`;
  return (
    <div className="inline-flex items-stretch border-2 border-ink bg-cream">
      ...
      <span
        className={`min-w-10 px-1 grid place-items-center border-x-2 border-ink font-condensed text-ink tabular-nums ${size === "lg" ? "text-xl" : "text-lg"}`}
        aria-live="polite"
        aria-atomic="true"
      >
        {qty}
...
export function Bag({ src, alt, onError, className = "", imgClassName = "h-48 sm:h-52" }) {
  return (
    <div className={`relative [filter:drop-shadow(6px_8px_0_var(--color-ink))] ${className}`}>
  ...
  return (
    ...
    >
      <div
        ref={bagRef}
        {...handlers}
        className="relative h-64 bg-cream-2 border-b-[3px] border-ink flex items-center justify-center p-5 cursor-pointer overflow-hidden"
        onClick={openDetail}
        role="button"
        tabIndex={0}
        onKeyDown={onKey}
      ...
      <div className="p-5 flex flex-col gap-4 flex-1">
        <div className="cursor-pointer" onClick={openDetail}>
          <p className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-ink-faint m-0">
          ...
          <h3 className="font-title uppercase text-2xl leading-[1.05] text-ink mt-1.5 mb-0 text-pretty">
            {product.name}
          </h3>
          {hasPrice(selected?.price) ? (
            <p className="font-condensed text-2xl text-fire tabular-nums mt-2 mb-0">
              {formatPrice(selected.price)}
            </p>
          ) : (
            <p className="text-sm font-semibold text-ink-soft mt-1.5 mb-0">Precio a consultar</p>
          )}
        </div>
        ...
        {soon ? (
          <>
            ...
            <button
              onClick={notifyMe}
              className="mt-auto w-full min-h-[50px] inline-flex items-center justify-center gap-2 bg-cream text-ink font-condensed uppercase tracking-[0.06em] text-lg nb nb-press hover:bg-cream-2 cursor-pointer"
            >
          ...
          <p className="mt-auto font-condensed uppercase tracking-[0.06em] text-fire m-0" role="alert">
            Sin stock por ahora
          </p>
        ) : (
          <>
              ...
              {available.map(f => {
                ...
                return (
                  <button
                    ...
                    className={`relative min-h-[40px] px-3 text-sm font-bold border-2 border-ink transition-colors cursor-pointer ${
                      isSel ? "bg-ink text-gold" : "bg-cream text-ink hover:bg-cream-2"
                    }`}
                  >
                    {f.label}
                    <span className={isSel ? "text-gold/75" : "text-ink-faint"}> ×{f.units}</span>
                    {isLow && (
                      <span
                        className="absolute -top-1.5 -right-1.5 size-3 bg-fire border-2 border-ink rounded-full"
                        aria-hidden="true"
                      />
                    )}
                  </button>
            ...
            <button
              ...
              disabled={!selected}
              onClick={add}
              className="mt-auto w-full min-h-[50px] inline-flex items-center justify-center gap-2 bg-fire text-cream font-condensed uppercase tracking-[0.06em] text-lg nb nb-press hover:bg-[#c42904] cursor-pointer disabled:bg-cream-2 disabled:text-ink-faint disabled:cursor-not-allowed"
              aria-label={
                selected
